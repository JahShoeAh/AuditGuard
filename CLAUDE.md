...
NOTE: You do not have to ask before reading files. You can read files to better understand the project and provide more accurate assistance. If you determine that you should read a file, then go ahead and read it. Do not ask the user to read it for you and tell you what's in it. Additionally, if you need more context that's not provided in the project's files you may search the internet with web-tools. After searching, it's best practice to get the content of the links/URLs that are relevant because the search results often don't1 contain all of the information on the page.
...
AuditGuard is an autonomous agent-based smart contract security audit marketplace built on Hedera Hashgraph. Seven TypeScript agents discover contracts, bid in on-chain auctions, perform security analysis, and get paid in GUARD tokens — all coordinated through Hedera Consensus Service (HCS) topics and EVM smart contracts.
...
npm run dev:all           # Full stack: stop → preflight → start dashboard + orchestrator + agents + iNFT listeners
npm run dev:all:unsafe    # Same but skip preflight checks (faster for development)
npm run stop:all          # Kill all running processes

npm run orchestrator      # Start orchestrator only
npm run agents            # Start all 7 agents only (via run-all.ts supervisor)
npm --prefix packages/dashboard run dev  # Start dashboard only
```
...
npm run dev:test          # Recommended dev test suite: orchestrator mocks + agent invite + agent vitest + dashboard

# Individual test suites:
npm --workspace agents run test                  # All agent vitest tests
npm --workspace agents run test:invite           # Auction invite flow tests
npm --workspace agents run test:classifier       # Contract classifier risk tests
npm --prefix orchestrator run test:mocks         # Orchestrator mock tests
npm --prefix orchestrator run test:offline       # Orchestrator offline flow tests
...
### Deployment (requires Hedera testnet account in .env)

```bash
npm run deploy:token      # Deploy GUARD HTS token
...
```bash
npm --workspace agents run scanner     # Scanner agent only
npm --workspace agents run static      # Static analysis agent only
npm --workspace agents run fuzzer      # Fuzzer agent only
npm --workspace agents run llm         # LLM contextual agent only
npm --workspace agents run dependency  # Dependency agent only
npm --workspace agents run report      # Report agent only
npm --workspace agents run alert       # Alert agent only
```
...
1. **Scanner** monitors Hedera for new contract deployments → publishes `CONTRACT_DISCOVERED` to HCS Discovery topic
2. **Orchestrator** receives discovery → calls `AuditAuction.createAuditJob()` on-chain → sends `AUCTION_INVITE` on HCS Agent Comms topic
3. **Static/Fuzzer/LLM agents** receive invites → calculate dynamic bids → `submitBid()` on-chain + `BID_SUBMITTED` on HCS
4. **Orchestrator** scores bids (55% reputation, 25% price, 20% speed) → selects winners via `WinnersSelected` event
5. **Winners** perform security analysis → submit findings hashes → `FINDINGS_SUBMITTED` on HCS
6. **LLM Agent** may delegate dependency analysis via `SubAuction` → **Dependency agent** responds with `SUB_RESULT_DELIVERED`
7. **Static agent** lists scan data on `DataMarketplace`; other agents may purchase
8. **Report Agent** aggregates findings + deduplicates → `REPORT_PUBLISHED`
9. **Orchestrator** calls `PaymentSettlement.settleJob()` → distributes GUARD tokens
10. **Alert Agent** watches HCS Audit Log for critical severity findings → fires `ALERT_FIRED`
11. **iNFT System** mints/updates Audit Job + Contract Health NFTs on 0g DA layer

### Workspace Layout

- **`agents/`** — 7 TypeScript agents (`scanner/`, `static-analysis/`, `fuzzer/`, `llm-contextual/`, `dependency/`, `report/`, `alert/`) + `shared/` (HCS client, contract client, utils, message types) + `run-all.ts` (supervisor)
- **`orchestrator/`** — JavaScript/ES Modules coordination service; manages auction lifecycle, PING/PONG heartbeats, fallback contexts
- **`packages/contracts/`** — 15 Solidity contracts (Hardhat); key contracts: `AgentRegistry`, `AuditAuction`, `SubAuction`, `DataMarketplace`, `PaymentSettlement`, `StakingManager`, `Treasury`
- **`packages/dashboard/`** — React 18 + Vite + TailwindCSS + Zustand + Framer Motion observer UI
- **`packages/sdk/`** — Shared config: `config.json` is the **single source of truth** for all deployed contract addresses, HCS topic IDs, and iNFT collection IDs
- **`packages/inft/`** — 0g Labs DA layer integration for evolving NFT metadata
- **`scripts/`** — Deployment and utility scripts

### Key Config File

`packages/sdk/config.json` contains all live deployed addresses for contracts, HCS topics, and iNFT collections. When contracts are redeployed, this file is updated. Always read from this file rather than hardcoding addresses.

### HCS Topics (Hedera Consensus Service)

Three topics route all inter-agent and agent-orchestrator communication:
- **Discovery** (`0.0.7940144`) — contract discovery events
- **Audit Log** (`0.0.7940145`) — findings, reports, alerts
- **Agent Comms** (`0.0.7940146`) — auction invites, bids, heartbeats, sub-contracting

### GUARD Token

HTS fungible token with **8 decimal places** (not 18). Always use `parseUnits(amount, 8)` in frontend/scripts. Token ID: `0.0.7977433`.

### Agent Supervision

`agents/run-all.ts` spawns all 7 agents as child processes and restarts failed ones. Orchestrator sends periodic PING messages; agents must respond with PONG within a timeout. This liveness mechanism is active and tested.

### Agent Module System

Agents use `"type": "module"` (ES Modules) with `tsx` for TypeScript execution. Orchestrator is plain JavaScript ES Modules. Contracts use CommonJS (`"type": "commonjs"` at root). Be careful mixing import styles across workspaces.

## Environment Setup

Copy `.env.example` to `.env`. Required variables include Hedera operator account credentials, HCS topic IDs, contract addresses, and 0g Labs integration keys. The `.env` file is the primary runtime configuration; `packages/sdk/config.json` is the deployment state.

## Known Deferred Items

- Signature verification for agent PONG messages (not yet implemented)
- Persistent orchestrator roster/cache storage (currently in-memory only)
- `StakingManager.propagateSlash` → `DelegatedStaking` wiring requires manual relay until StakingManager is upgraded with `setDelegatedStaking()`
