...
function resolveDonorKeys(): string[] {
  ...
  const candidates = custom.length > 0 ? custom : defaults;
...
export async function ensureBidCollateralBalance(
  ...
  return {
    ...
    attemptedTopUp: true,
    toppedUpWei,
    reason: "Insufficient GUARD balance for bid collateral after auto top-up attempt",
  };
}
...
export function getBidCollateralTopUpConfig() {
  return {
    ...
    donorsConfigured: resolveDonorKeys().length,
