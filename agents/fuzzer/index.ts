...
async function main() {
  ...
  // Listen for data listings from other agents
  ...
  // Listen for contract discovery events
  hcs.subscribeDiscovery(async (msg: HCSMessage) => {
    ...
    log.info(
      `Bidding: ${bid.amount} GUARD (collateral: ${bid.collateral}) ` +
  ...
  log.info("Subscribed to discovery + agent comms. Waiting for contracts...");
}
