...
function PortfolioSummary({ totalDelegated, totalRewards, count, onClaimAll, isClaiming }) {
  return (
    <div className="flex items-center gap-4 flex-wrap px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg mb-3">
      <div className="flex-1 min-w-0">
        ...
        <span className="text-sm font-bold font-mono text-yellow-400">{totalRewards} GUARD</span>
        <span className="mx-3 text-gray-600">│</span>
...
function DelegationCard({ agentAddr, amount, pendingRewards, poolData, agentProfile, onAddMore, onRefresh }) {
...
export default function DelegationPortfolio({ onSelectAgent }) {
  ...
  const handleClaimAll = async () => {
    ...
    try {
      ...
      toast.error(`✗ Claim all failed: ${err?.message?.slice(0, 80) ?? 'unknown'}`);
    } finally {
      resetClaimAll();
    }
  };
