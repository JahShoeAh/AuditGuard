...
function ReportCard({ report }) {
  ...
  // Only fetch full report (with mdContent) when the user expands the card
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden hover:border-cyan-500/40 transition-all">
      ...
      <div className="p-4 space-y-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {Object.entries(sev).map(([s, count]) =>
              count > 0 ? (
                <span
                  key={s}
                  className={`text-[9px] font-bold px-2 py-0.5 rounded ${SEV_CLASSES[s] ?? SEV_CLASSES.info}`}
                >
                  {s.toUpperCase()}: {count}
                </span>
              ) : null,
            )}
            {Object.values(sev).every((c) => c === 0) && (
        ...
        <div className="flex items-center justify-between text-[10px] font-mono text-gray-500 border-t border-gray-800 pt-2">
          <span>
            Total findings:{' '}
            <span className="text-gray-300 font-bold">{report.findingCount ?? 0}</span>
          </span>
          <span>
            Agents:{' '}
            <span className="text-gray-300 font-bold">{report.agentCount ?? 0}</span>
          </span>
        </div>
        ...
        {report.tags?.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {report.tags.map((t, i) => (
              <span key={i} className="text-[9px] text-gray-600 bg-gray-800 px-1.5 py-0.5 rounded font-mono">
                #{t}
              </span>
            ))}
          </div>
        ...
        {/* Expanded content — lazy-fetched mdContent */}
        {expanded && (
          <div className="bg-gray-950 rounded border border-gray-800 p-3 max-h-64 overflow-y-auto text-[11px] font-mono text-gray-300">
            ...
            {!fullLoading && !fullError && !full?.mdContent && (
              <p className="text-gray-600 italic">
                Report content unavailable (no S3 bucket configured in local dev).
              </p>
            )}
          </div>
        )}
